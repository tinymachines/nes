...
//! tests/joypad.rs). The pad is set from PAD=hh (the same byte every
//! frame) or a script file of `AT <frame> <hh>` lines; an unscripted
//! frame keeps the last byte. A summary line closes: polls, the
//! histogram of clocks per poll, and the latch indices that took nine.
//!
...
fn main() {
    ...
    let mut script: Vec<(usize, u8)> = Vec::new();
    if let Some(path) = args.get(3) {
        for line in std::fs::read_to_string(path).expect("script").lines() {
            let f: Vec<&str> = line.split_whitespace().collect();
            if f.len() == 3 && f[0] == "AT" {
                script.push((f[1].parse().expect("frame"), u8::from_str_radix(f[2], 16).expect("hex byte")));
            }
        }
    }
    ...
    for frame in 0..frames {
        if let Some(&(_, b)) = script.iter().find(|(f, _)| *f == frame) {
            pad = b;
        }
