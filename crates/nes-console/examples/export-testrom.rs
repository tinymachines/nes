...
//! With `bars`, the colour-bars cartridge (testrom::bars_program) instead.
use nes_console::testrom::{bars_chr, bars_program, chr, program};

fn main() {
    let out = std::env::args().nth(1).unwrap_or_else(|| "testcart.nes".into());
    let bars = std::env::args().nth(2).as_deref() == Some("bars");
    let (prg, chr) = if bars { (bars_program(), bars_chr()) } else { (program(), chr()) };
